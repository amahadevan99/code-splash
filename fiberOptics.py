...
author = "reed essick and Alvaro the spanish wonder"
...
#        ### an equivalent block of code
    ...
    def readFromTerminal(self):
        ### a method to read in columns from the command line
...
def getFiberInDataPoint(dataPoint, fiber):
    ...
    if fiber not in known_fibers:
        print "Sorry I do not know this fiber"

    else:
        for fib in known_fibers_setup:
            if fib in dataPoint.data.keys() and dataPoint.data[fib] == fiber:
                fiberInDataPointInfo = getFiberInfo(asignateNumber(fib), dataPoint)
            else:
                pass

    return fiberInDataPointInfo

# goes thru a list of dataPoints extracting the information of a particular fiber
...
def plotMe(data):
    ...
    #    ax_H = plt.subplot(2,1,1) #2rows 1 column 1at panel
...
while True:
    ...
    ### figure out what el matador wants to do
